...
function accessLabel(result: SteamSearchResult) {
  ...
  if (!game) return "Steam · todavía no está en GameAccess";
    ...
    <div className="global-search" onFocus={() => setFocused(true)} onBlur={() => window.setTimeout(() => setFocused(false), 140)}>
      ...
      {open ? (
        <section className="global-search-popover" aria-label="Resultados de Steam">
            ...
            {shown.map((result) => {
              ...
              return (
                <button
                  ...
                  onClick={() => result.catalog_game && onOpenCatalogGame(result.catalog_game)}
                  title={inPool ? `Abrir ${result.name}` : `${result.name} está en Steam pero todavía no forma parte del pool`}
                >
          ...
          <div className="global-search-foot">La búsqueda cubre Steam completo. La disponibilidad de GameAccess se muestra por separado.</div>
        </section>
      ) : null}
    </div>
  );
