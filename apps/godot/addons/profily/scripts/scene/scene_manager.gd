...
## Scene stats module root (Godot-specific module). Same state machine as the
## RAM module: TEXT and BASIC are identical.
## (c) 2026 Javier Garrido (nodlag), MIT.

const SceneGraph := preload("scene_graph.gd")
const SceneMonitor := preload("scene_monitor.gd")
const SceneText := preload("scene_text.gd")

@onready var _monitor: SceneMonitor = %Monitor
@onready var _bg_full: Panel = %BackgroundFull
@onready var _bg_text: Panel = %BackgroundText
@onready var _graph: SceneGraph = %Graph
@onready var _text: SceneText = %Text
...
func _initial_state() -> ProfilyTypes.ModuleState:
	return _manager.scene_module_state
