...
var _manager: GodotxToastManager = null
...
func _initialize():
	...
	_manager = GodotxToastManager.new()
	_manager.name = "GodotxToastManager"
	add_child(_manager)
...
func show(
	...
	if message.is_empty():
		push_error("GodotX Toast: Message cannot be empty")
		return ""
	
	var resolved_style: GodotxToastStyle
	if style == null:
		resolved_style = GodotxToastStyles.get_style(GodotxToastStyles.INFO)
	elif style is String:
		resolved_style = GodotxToastStyles.get_style(style)
	elif style is GodotxToastStyle:
		resolved_style = style
	else:
		push_error("GodotX Toast: Invalid style type")
		return ""
	
	if resolved_style == null:
		push_error("GodotX Toast: Could not resolve style")
		return ""
...
func show_loading(
	...
	if message.is_empty():
		push_error("GodotX Toast: Loading message cannot be empty")
		return ""
	
	var resolved_style: GodotxToastStyle
	if style == null:
		resolved_style = GodotxToastStyles.get_style(GodotxToastStyles.LOADING)
	elif style is String:
		resolved_style = GodotxToastStyles.get_style(style)
	elif style is GodotxToastStyle:
		resolved_style = style
	else:
		push_error("GodotX Toast: Invalid style type for loading")
		return ""
	
	if resolved_style == null:
		push_error("GodotX Toast: Could not resolve loading style")
		return ""
...
func clear_by_origin(origin: GodotxToastEnums.ToastOrigin):
	if not _initialized or _manager == null:
		return
...
func success(message: String, time: Variant = null) -> String:
	return show(message, GodotxToastStyles.SUCCESS, time if time != null else default_time)

func error(message: String, time: Variant = null) -> String:
	return show(message, GodotxToastStyles.ERROR, time if time != null else default_time)

func warning(message: String, time: Variant = null) -> String:
	return show(message, GodotxToastStyles.WARNING, time if time != null else default_time)

func info(message: String, time: Variant = null) -> String:
	return show(message, GodotxToastStyles.INFO, time if time != null else default_time)
