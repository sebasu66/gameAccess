class_name GodotxToastLoadingView
extends GodotxToastView
...
func _setup_spinner():
	...
	match style.spinner_type:
		GodotxToastEnums.SpinnerType.NONE:
			return
		GodotxToastEnums.SpinnerType.DEFAULT:
			_spinner = _create_default_spinner()
		GodotxToastEnums.SpinnerType.TEXTURE:
			if style.spinner_texture:
				_spinner = _create_texture_spinner()
			else:
				push_error("GodotxToastLoadingView: spinner_type is TEXTURE but spinner_texture is null")
				return
		GodotxToastEnums.SpinnerType.SCENE:
			if style.spinner_scene:
				_spinner = _create_scene_spinner()
			else:
				push_error("GodotxToastLoadingView: spinner_type is SCENE but spinner_scene is null")
				return
	...
	if _spinner and _icon_container:
		...
		push_error("GodotxToastLoadingView: failed to create spinner")
...
func _create_scene_spinner() -> Control:
	...
	if not instance is Control:
		push_error("GodotxToastLoadingView: spinner_scene root must be a Control")
		instance.queue_free()
		return null
...
func complete(success: bool = true, final_message: String = ""):
	...
	dismiss(GodotxToastEnums.ToastDismissReason.API)
