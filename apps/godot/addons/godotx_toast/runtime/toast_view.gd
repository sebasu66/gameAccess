## GodotxToastView — single toast notification node.
##
...
var toast_id: String
var style: GodotxToastStyle
var message: String
var display_time: float
var origin: GodotxToastEnums.ToastOrigin
var animation: GodotxToastEnums.ToastAnimation
...
func _setup_background():
	...
	match style.background_type:
		GodotxToastEnums.BackgroundType.NONE:
		...
		GodotxToastEnums.BackgroundType.COLOR:
		...
		GodotxToastEnums.BackgroundType.TEXTURE:
			if style.background_texture:
				...
				push_error("GodotxToastView: background_type TEXTURE but background_texture is null")

		GodotxToastEnums.BackgroundType.SCENE:
			if style.background_scene:
				...
				if inst is Control:
					...
					push_error("GodotxToastView: background_scene root must be a Control")
					inst.queue_free()
			else:
				push_error("GodotxToastView: background_type SCENE but background_scene is null")
...
func _apply_style():
	var font_to_use: Font = style.font if style.font else GodotxToastStyles.get_default_font()
	if font_to_use:
...
func _setup_icon():
	...
	if style.icon_scene:
		...
		if inst is Control:
			...
			push_error("GodotxToastView: icon_scene root must be a Control")
			inst.queue_free()
...
func _setup_close_button():
	...
	if style.close_button_scene:
		...
		if inst is Control:
			...
			push_error("GodotxToastView: close_button_scene root must be a Control")
			inst.queue_free()
		...
		if event.pressed:
			...
			if _is_dragging:
				...
				if not did_swipe and style.tap_to_dismiss:
					...
					dismiss(GodotxToastEnums.ToastDismissReason.CLICK)
...
func _on_close_input(event: InputEvent):
	...
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		...
		dismiss(GodotxToastEnums.ToastDismissReason.CLOSE_BUTTON)
...
func _update_drag_position():
	...
	match style.swipe_direction:
		GodotxToastEnums.SwipeDirection.HORIZONTAL:
			position.x = _drag_start_pos.x + delta.x
		GodotxToastEnums.SwipeDirection.VERTICAL:
			position.y = _drag_start_pos.y + delta.y
		GodotxToastEnums.SwipeDirection.BOTH:
...
func _swipe_out(direction: Vector2):
	...
	_tween.tween_callback(func():
		...
		_complete_dismiss(GodotxToastEnums.ToastDismissReason.SWIPE)
	)
...
func _on_timeout():
	if not _is_dismissing:
		dismiss(GodotxToastEnums.ToastDismissReason.TIMEOUT)

# ---------------------------------------------------------------------------
# Dismiss
# ---------------------------------------------------------------------------

func dismiss(reason: int = GodotxToastEnums.ToastDismissReason.API):
	if _is_dismissing:
		return
	_is_dismissing = true
	if _timer:
...
func _animate_out(reason: int):
	if _tween and _tween.is_valid():
		_tween.kill()
	if animation == GodotxToastEnums.ToastAnimation.NONE:
...
func _slide_in_start(target: Vector2) -> Vector2:
	...
	match origin:
		...
		GodotxToastEnums.ToastOrigin.RIGHT, GodotxToastEnums.ToastOrigin.TOP_RIGHT, GodotxToastEnums.ToastOrigin.BOTTOM_RIGHT:
			s.x = vp.x + 50
		GodotxToastEnums.ToastOrigin.TOP:
			s.y = -size.y - 50
		_:  # BOTTOM, CENTER
			s.y = vp.y + 50
	return s
...
func update_style(new_style: GodotxToastStyle, new_ui_scale: float = 1.0):
	style     = new_style
