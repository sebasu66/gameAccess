class_name GodotxToastContainer
extends Control

var origin: GodotxToastEnums.ToastOrigin
var _toasts: Array[GodotxToastView] = []
var _queued_toasts: Array[Dictionary] = []
var _spacing: int = 8

func _init(p_origin: GodotxToastEnums.ToastOrigin):
	origin = p_origin
	mouse_filter = MOUSE_FILTER_IGNORE
	z_index = 200
	anchors_preset = Control.PRESET_FULL_RECT

func add_toast(toast_view: GodotxToastView, strategy: int, max_visible: int) -> bool:
	if _toasts.size() >= max_visible:
		match strategy:
			GodotxToastEnums.StackStrategy.DROP_NEW:
				toast_view.queue_free()
				return false
			GodotxToastEnums.StackStrategy.DROP_OLDEST:
				if _toasts.size() > 0:
					_toasts[0].dismiss(GodotxToastEnums.ToastDismissReason.API)
				_toasts.append(toast_view)
			GodotxToastEnums.StackStrategy.REPLACE_OLDEST:
				if _toasts.size() > 0:
					_toasts[0].dismiss(GodotxToastEnums.ToastDismissReason.API)
				_toasts.append(toast_view)
			GodotxToastEnums.StackStrategy.QUEUE:
...
func _add_toast_node(toast_view: GodotxToastView):
...
func _position_toast_after_layout(toast_view: GodotxToastView):
...
func _get_toast_position(toast_view: GodotxToastView) -> Vector2:
...
func _on_toast_swiped(toast_id: String, _direction: Vector2):
	dismiss_toast(toast_id, GodotxToastEnums.ToastDismissReason.SWIPE)
...
func _calculate_positions() -> Array:
	...
	var visible_toasts: Array[GodotxToastView] = []
...
func _calculate_x_for_toast(viewport_size: Vector2, safe_area: Dictionary, toast_width: float) -> float:
	match origin:
		GodotxToastEnums.ToastOrigin.CENTER, GodotxToastEnums.ToastOrigin.TOP, GodotxToastEnums.ToastOrigin.BOTTOM:
			return (viewport_size.x - toast_width) / 2.0
		GodotxToastEnums.ToastOrigin.LEFT, GodotxToastEnums.ToastOrigin.TOP_LEFT, GodotxToastEnums.ToastOrigin.BOTTOM_LEFT:
			return safe_area.left + 16.0
		GodotxToastEnums.ToastOrigin.RIGHT, GodotxToastEnums.ToastOrigin.TOP_RIGHT, GodotxToastEnums.ToastOrigin.BOTTOM_RIGHT:
			return viewport_size.x - safe_area.right - 16.0 - toast_width
	return 0.0

func _get_visible_toasts() -> Array[GodotxToastView]:
	var visible: Array[GodotxToastView] = []
	for toast in _toasts:
		if not toast._is_dismissing:
			visible.append(toast)
	return visible

func _get_anchor_x(viewport_size: Vector2, safe_area: Dictionary, visible_toasts: Array[GodotxToastView] = []) -> float:
	if visible_toasts.is_empty():
...
func _get_anchor_y(viewport_size: Vector2, safe_area: Dictionary, visible_toasts: Array[GodotxToastView] = []) -> float:
	if visible_toasts.is_empty():
		visible_toasts = _get_visible_toasts()
	...
	match origin:
		GodotxToastEnums.ToastOrigin.TOP, GodotxToastEnums.ToastOrigin.TOP_LEFT, GodotxToastEnums.ToastOrigin.TOP_RIGHT:
			return safe_area.top + 16.0
		GodotxToastEnums.ToastOrigin.BOTTOM, GodotxToastEnums.ToastOrigin.BOTTOM_LEFT, GodotxToastEnums.ToastOrigin.BOTTOM_RIGHT:
			if visible_toasts.is_empty():
		...
		GodotxToastEnums.ToastOrigin.CENTER:
			return (viewport_size.y - total_height) / 2.0
		GodotxToastEnums.ToastOrigin.LEFT, GodotxToastEnums.ToastOrigin.RIGHT:
			return (viewport_size.y - total_height) / 2.0
	return 16.0
...
func get_toast_by_id(id: String) -> GodotxToastView:
	for toast in _toasts:
		if toast.toast_id == id:
			return toast
	return null

func dismiss_all(reason: int = GodotxToastEnums.ToastDismissReason.API):
...
func dismiss_toast(id: String, reason: int = GodotxToastEnums.ToastDismissReason.API) -> bool:
	for toast in _toasts:
		if toast.toast_id == id:
			toast.dismiss(reason)
			return true
