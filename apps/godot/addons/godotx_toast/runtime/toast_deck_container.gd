## GodotxToastDeckContainer — Sonner-style stacked deck.
##
## Architecture:
...
class_name GodotxToastDeckContainer
extends GodotxToastContainer

const MAX_PREVIEWS: int = 2

## Cards currently in the scene (indices 0 = front active, 1+ = previews).
var _visible_cards: Array[GodotxToastView] = []
## Cards waiting — not yet added to the scene.
var _deck_queue: Array[GodotxToastView] = []
...
func add_toast(toast_view: GodotxToastView, strategy: int, _max_visible: int) -> bool:
	if strategy != GodotxToastEnums.StackStrategy.DECK:
...
func _push_to_visible(toast_view: GodotxToastView):
...
func _enter_preview(toast_view: GodotxToastView):
...
func _exit_preview(toast_view: GodotxToastView):
...
func _scale_for_slot(slot: int, style: GodotxToastStyle) -> float:
...
func _pos_for_slot(active_pos: Vector2, _h: float, slot: int,
		_scale: float, style: GodotxToastStyle) -> Vector2:
...
func _compute_active_position(toast_view: GodotxToastView) -> Vector2:
	...
	match origin:
		GodotxToastEnums.ToastOrigin.BOTTOM:
			return Vector2((vp.x - w) / 2.0,            vp.y - sa.bottom - pad - h)
		GodotxToastEnums.ToastOrigin.BOTTOM_LEFT:
			return Vector2(sa.left + pad,                vp.y - sa.bottom - pad - h)
		GodotxToastEnums.ToastOrigin.BOTTOM_RIGHT:
		...
		GodotxToastEnums.ToastOrigin.TOP_LEFT:
			return Vector2(sa.left + pad,                sa.top + pad)
		GodotxToastEnums.ToastOrigin.TOP_RIGHT:
			return Vector2(vp.x - sa.right - pad - w,   sa.top + pad)
		GodotxToastEnums.ToastOrigin.LEFT:
			return Vector2(sa.left + pad,                (vp.y - h) / 2.0)
		GodotxToastEnums.ToastOrigin.RIGHT:
